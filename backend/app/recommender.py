...
class ContentRecommender:
    ...
    def generate(self, usuario_id: int) -> Dict:
        ...
        try:
            ...
            # Extract the output string from the first task's result
            if not results.tasks_results:
                raise ValueError("No task results available.")

            output = results.tasks_results[0].result

            if not output:
                raise ValueError("No output generated by the agent.")
